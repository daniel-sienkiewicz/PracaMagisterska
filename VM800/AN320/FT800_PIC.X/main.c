 ...
 * Processor:       Microchip PIC16F886 @ 8MHz
 ...
 **************************************************************************
 * SM		2014-15-04	1.0		Initial version
 **************************************************************************
 */
...
void ft800memWrite32(unsigned long ftAddress, unsigned long ftData32)
{
  ...
  while(!SSPSTATbits.BF);
  SSPBUF = ((char)(ftData32 >> 8));
  while(!SSPSTATbits.BF);
  SSPBUF = ((char)(ftData32 >> 16));
  while(!SSPSTATbits.BF);
  SSPBUF = ((char)(ftData32 >> 24));
  while(!SSPSTATbits.BF);
  __delay_us(2);// Send data high byte
  RC6 = 1;
}
...
unsigned char ft800memRead8(unsigned long ftAddress)
{
  ...
  while(!SSPSTATbits.BF);
 SSPBUF = ZERO;
 while( (SSPSTAT & (1<<0)) == 0);// Send dummy byte
    ftData8 = SSPBUF;
 //   while(!SSPSTATbits.BF);
 //   __delay_us(2);// Read data byte
  RC6 = 1;
...
*/
  ...
  //  Serial.write("System Halted!\r\n");		// Send an error message on the UART
   //EWNETUALNIE WYWAKLI 
    SSPBUF = 0xFF;
    while(!SSPSTATbits.BF);// If we don't get 0x7C, the ineface isn't working - halt with infinite loop
  }
...
// ftRead =  ft800memRead32(RAM_CMD + cmdOffset);
 ...
 //   ftRead = ft800memRead32(RAM_CMD + cmdOffset);// Set the color of the following item(s) - toggle red/white from above
  ...
  __delay_ms(500);					// Wait a half-second to observe the changing color

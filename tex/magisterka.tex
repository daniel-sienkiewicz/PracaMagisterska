...
Rozpoczęcie pracy wyświetlacza polega na wpisaniu określonych wartości do pewnych obszarów jego pamięci określając w ten sposób np. rozdzielczość lub włączenie/wyłącznie modułu odpowiedzialnego za dźwięk czy dotyk. Pamięć w systemie możemy zobrazować sobie jako tablica, w której każda komórka ma swój adres. Wysłanie pewnej wartości do pamięci oznacza więc wpisanie tej wartości do określonej przez adres komórki w tablicy.
...
\begin{enumerate}
	\item 7 przełączników bistabilnych symulujących drzwi samochodu (wraz z bagażnikiem), wrzucenie biegu wstecznego oraz włączenie świateł w samochodzie
...
Są to bezpośrednie funkcje systemu lub sprzętu ułatwiające komunikację ze światem zewnętrznym. Część z nich jest zarezerwowana przez system lecz część jest wolna do wykorzystania przez programistę. Przerwania możemy podzielić na trzy podstawowe rodzaje:
...
W środowisku \emph{Arduino} aby obsłużyć port analogowy wystarczy ustalić tryb w jakim ma on działać (wejście/wyjście), a następnie wysłać/odczytać dane.  Wpisanie wartości \emph{LOW} jest równoznaczna z wysyłaniem napięcia 0V na określonym pinie, a wpisanie wartości \emph{HIGH} jest równoznaczna z wysyłaniem napięcia 5V na określonym pinie. Dla portu cyfrowego wygląda to następująco:
...
Dla portów analogowych jest podobnie z jedną różnicą: zamiast wysyłać/odczytywać wartości HIGH oraz LOW możemy wysłać/odczytać dowolną liczbę 10 bitową.
...
%==== TUTAJ co to jest start i STOP plus rysunek!!!!!
...
%==Obsługa I2C problem z WIRE. Standardowy kontrolec I2C w galileo jest wyposazony w sprzetowy interfejs I2C ktory obsługiwany sprzetowo. W zwiazku z tym napisałem swoje.!!!!
...
%== moja ocena, nie ma pełnej funkcjonalności systemu!!!!!!
